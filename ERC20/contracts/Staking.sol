...
contract Staking is Ownable {
    ...
    function initialize() public {
        ...
        require(token.transferFrom(msg.sender, address(this), tokenForStake), "Staking: transfer failed!");
        isInitialize = true;
    }
    ...
    function stakeToken(uint256 amount) public {
        ...
        require(token.transferFrom(msg.sender, address(this), amount), "Staking: stake failed!");
    }

    function claimToken() public {
        require(userStakeInfo[msg.sender].dayEnd < block.timestamp, "Staking: not enough 30 days!");

        uint256 bonusToken = userStakeInfo[msg.sender].stakeAmount * token.allowance(owner(), address(this)) / totalStakeAmount;
        uint256 totalTokenClaim = userStakeInfo[msg.sender].stakeAmount + bonusToken;
        require(token.transfer(msg.sender, totalTokenClaim), "Staking: claim failed!");
